	...
	function checkDragAndDropAPI(){
		...
		*/
		bSupportDragAndDropAPI = false;		// html5 파일업로드 안되 일단 막아놓음.
	}
    ...
    function html5Upload() {	
    	...
    	sUploadURL= 'editor_upload_html5.jsp'; 	//upload URL
    ...
    function callAjaxForHTML5 (tempFile, sUploadURL){
    	...
    	oAjax.header("Content-Type","multipart/form-data: boundary= ----------------------------------------7db8c16240665");
...
// 	    		oCustomEvent.stop(); 수행시 bAllowed 가 false이더라도 alert이 수행되지 않음
	 ...
	 */
 	function setPhotoToEditor(oFileInfo){
 		...
 		if(document.location.host == '' || document.location.host == undefined){
 			url = '';
 		}
		if (!!opener && !!opener.nhn && !!opener.nhn.husky && !!opener.nhn.husky.PopUpManager) {
			...
			var cntRVS = oFileInfo.length -1;
			for(var i = 0;i < oFileInfo.length; i++){
				var img = "<img";
				img = img + " title='" + oFileInfo[cntRVS - i].sFileName + "'";
				img = img + " src='" + url + oFileInfo[cntRVS - i].sFileURL + "'";
				img = img + " alt='" + oFileInfo[cntRVS - i].alt + "'";
				//img = img + " style='max-width:" + oFileInfo[cntRVS - i].width + "; height:auto;'";
				img = img + " />";
				if(oFileInfo[cntRVS - i].bNewLine){
