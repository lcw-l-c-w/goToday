...
public interface ContentReplyMapper {
	...
	// 답변여부 체크
	int updateReplyStatus(@Param("gno") int gno, @Param("reply_status") int reply_status);
	...
	int updateReply(ContentReplyVO vo);
	  ...
	  List<ContentReplyVO> selectQuestionPage(
		      @Param("content_id") int content_id,
		      @Param("offset") int offset,
		      @Param("limit") int limit
		  );

		  int countQuestion(@Param("content_id") int content_id);

		  List<ContentReplyVO> selectAnswersByGno(@Param("gno") List<Integer> gno);
